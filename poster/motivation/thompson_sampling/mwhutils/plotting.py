...
class Axis(object):
    ...
    def scatter(self, x, y, label='', color = "black", zorder = 1, s = 40, marker = None):
        """
        Add a scatter plot to the axis.
        """
        if marker is None:
            marker = next(self._markers)
        self._ax.scatter(x, y, lw=1.5, color= color,
                         s= s, facecolors='none', label=label, zorder = zorder, marker = marker)
        self._draw()
    ...
    def plot(self, x, y=None, lo=None, hi=None, add=True, label='', zorder = 1, color = None):
        """
        ...
        self._ax.plot(x, y, ls='-', color=color, dashes=dashes, label=label, lw=PARAMS['weight'], zorder = zorder)
    ...
    def vline(self, x, label='', color = 'green', ymax = 1.0 / 1.2):
        """
        Add a vertical line to the axis at x.
        """
        
#        color = next(self._lcolors)
        self._ax.axvline(x, ymax = ymax, ls='--', lw=4, color=color, label=label)
        self._draw()
    ...
    def save(self, fname):
        self._ax.figure.savefig(fname, axis='tight')
...
class Figure(object):
    ...
    def save(self, fname):
        self._fig.savefig(fname, axis='tight')
